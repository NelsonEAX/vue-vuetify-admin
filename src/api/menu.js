/* Не используется. Меню строиться динамически на основе Routes */
const Menu = [
  { header: 'Apps' },
  {
    title: 'Dashboard',
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'jumbotrons', title: 'Jumbotrons', component: 'components/jumbotrons' },
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'selects', title: 'Selects', component: 'components/selects' },
...
Menu.forEach(item => {
  if (item.items) {
    item.items.sort((x, y) => {
      ...
      const ternary = (textA > textB) ? 1 : 0;
      return (textA < textB) ? -1 : ternary;
    });
  }
});
